...
void Lab2::Init()
{
    ...
    {
        vector<VertexFormat> vertices{
            // TODO(student): Complete the vertices data for the tetrahedron mesh
            VertexFormat(glm::vec3(-1, 0, -1), VertexColor::RED), // 0
            VertexFormat(glm::vec3(1, 0, -1), VertexColor::GREEN), // 1
            VertexFormat(glm::vec3(0, 0, 1), VertexColor::BLUE), // 2
            VertexFormat(glm::vec3(0, 1, 0), VertexColor::WHITE) // 3
        };

        vector<unsigned int> indices{
            // TODO(student): Complete indices data for the tetrahedron mesh
            0, 1, 2, // base triangle
            0, 3, 1, // south triangle
            1, 3, 2, // east triangle
            2, 3, 0, // west triangle
        };

        meshes["tetrahedron_ex4"] = new Mesh("generated tetrahedron ex4");
        meshes["tetrahedron_ex4"]->InitFromData(vertices, indices);

        // Actually create the mesh from the data
        CreateMesh("tetrahedron_ex4", vertices, indices);
    }
    ...
    // opposing vertex orientations.
    {
        vector<VertexFormat> vertices{
            // TODO(student): Complete the vertices data for the cube mesh
            VertexFormat(glm::vec3(-1, 0, -1), VertexColor::WHITE), // 0
            VertexFormat(glm::vec3(1, 0, -1), VertexColor::WHITE), // 1
            VertexFormat(glm::vec3(1, 0, 1), VertexColor::WHITE), // 2
            VertexFormat(glm::vec3(-1, 0, 1), VertexColor::WHITE) // 3
        };
        ...
        meshes["rectangle_ex6"] = new Mesh("generated rectangle ex6");
        meshes["rectangle_ex6"]->InitFromData(vertices, indices);
...
void Lab2::CreateMesh(const char* name, const std::vector<VertexFormat>& vertices,
                      const std::vector<unsigned int>& indices)
{
    ...
    if (GetOpenGLError() == GL_INVALID_OPERATION) {
        cout
            << "\t[NOTE] : For students : DON'T PANIC! This error should go away when completing the tasks."
            << std::endl;
        cout
            << "\t[NOTE] : For developers : This happens because OpenGL core spec >=3.1 forbids null VAOs."
            << std::endl;
    }
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_A"], shaders["VertexColor"], glm::vec3(-1.5f, 0.5f, 0),
               glm::vec3(0.25f));

    // TODO(student): Draw the mesh that was created with `CreateMesh()`

    // TODO(student): Draw the tetrahedron
    RenderMesh(meshes["tetrahedron_ex4"], shaders["VertexColor"], glm::vec3(5, 0, 5));
