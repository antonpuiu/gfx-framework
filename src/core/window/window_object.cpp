...
void WindowObject::FullScreen()
{
    GLFWmonitor* monitor = glfwGetPrimaryMonitor();
    const GLFWvidmode* videoDisplay = glfwGetVideoMode(monitor);
    window->handle = glfwCreateWindow(videoDisplay->width, videoDisplay->height, props.name.c_str(),
                                      monitor, NULL);
...
void WindowObject::WindowMode()
{
    ...
    window->handle =
        glfwCreateWindow(props.resolution.x, props.resolution.y, props.name.c_str(), NULL, NULL);
    assert(window->handle != nullptr);
    glfwMakeContextCurrent(window->handle);
...
void WindowObject::UpdateObservers()
{
    ...
    if (scrollEvent) {
        scrollEvent = false;
        for (auto obs : observers) {
            obs->OnMouseScroll(props.cursorPos.x, props.cursorPos.y, mouseScrollDeltaX,
                               mouseScrollDeltaY);
        }
    }
    ...
    if (registeredKeyEvents) {
        for (int i = 0; i < registeredKeyEvents; i++) {
            for (auto obs : observers)
                keyStates[keyEvents[i]] ? obs->OnKeyPress(keyEvents[i], keyMods) :
                                          obs->OnKeyRelease(keyEvents[i], keyMods);
        }
        registeredKeyEvents = 0;
    }
